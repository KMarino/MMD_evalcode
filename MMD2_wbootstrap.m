...
% Calculate over bootstraps
bs_size = 10000;
bs_mmdvals = [];
for bs_count = 1:bs_size
    if mod(bs_count, 100) == 0
        disp(bs_count)
    end

    % Get bootstrap sample
    norm1_bs = datasample(norm1(:), m^2);
    norm2_bs = datasample(norm2(:), n^2);
    norm3_bs = datasample(norm3(:), m*n); 

    mmd2vals = zeros(size(hs));
    for i = 1:length(hs)
        h = hs(i);
        sum1vals = exp(-norm1_bs.^2/(2*h^2));
        sum1 = sum(sum(sum1vals));
        sum1 = sum1 / (m*(m-1));
        sum2vals = exp(-norm2_bs.^2/(2*h^2));
        sum2 = sum(sum(sum2vals));
        sum2 = sum2 / (n*(n-1));
        sum3vals = exp(-norm3_bs.^2/(2*h^2));
        sum3 = sum(sum(sum3vals));
        sum3 = (sum3*2)/(m*n);
        mmd2 = sum1 + sum2 - sum3;
        mmd2vals(i) = mmd2;
    end
    mmd2val = max(mmd2vals);
    bs_mmdvals = [bs_mmdvals, mmd2val];
end
...
disp(['MMD^2 mean: ' num2str(mean(bs_mmdvals))])
disp(['MMD^2 var: ' num2str(std(bs_mmdvals))])
